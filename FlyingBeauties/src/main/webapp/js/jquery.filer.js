			...
			var s = $(r),
				...
				f = {
					...
					_thumbCreator: {
						...
						renderFile: function(file, html, opts) {
							...
							if(file.file && opts.type == "image") {
								var g = '<img src="' + file.file + '" draggable="true" />',
								...
								if(fileTypes.indexOf(extension) > -1) {
									...
									fr.onload = function(e) {
								        ...
								        $(tiffCanvas).css({
								            ...
								            "display": "block",
								            "padding-top": "10px"
								        }).addClass("preview").attr('id', 'canvas');;
								        
								        $(".jFiler-item-thumb-image").append(tiffCanvas);
								        $(".jFiler-item-thumb-image").removeClass('fi-loading');
								        
								        var canvas = document.getElementById("canvas");
								        var jpegUrl = canvas.toDataURL("image/jpeg");
								        
								        var base64Code = jpegUrl.split(",")[1];
								        $("#fileBase64Code").val(base64Code);
								        $("#originalImageName").val(file.name);
							        }
									...
									y.onload = function(e) {
										var g = '<img src="' + e.target.result + '" draggable="true" />',
											m = html.find('.jFiler-item-thumb-image.fi-loading');
										$(g)
											.error(function() {
												g = f._thumbCreator.generateIcon(opts);
									...
									/* ThienMV Custom START */
									var reader1 = new FileReader;
									 reader1.onload = function(e){
										 var binaryString = e.target.result;
								         $("#fileBase64Code").val(btoa(binaryString));
								         $("#originalImageName").val(file.name);
									 };
					...
					_remove: function(e, el) {
						...
						/* ThienMV Custom START */
						/* Remove value when delete image */
						$("#fileBase64Code").val("");
						$("#locationName").val("");
						$("#lngHdn").val("");
						$("#latHdn").val("");
